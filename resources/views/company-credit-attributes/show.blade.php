...
@section('title', ($companyCreditAttribute->name ?: 'Company credit attribute').' archive')
@section('meta_description', 'Browse published company credit records, linked companies, categories, and titles that use the '.($companyCreditAttribute->name ?: 'selected').' attribute.')

@section('breadcrumbs')
    <x-ui.breadcrumbs.item :href="route('public.home')">Home</x-ui.breadcrumbs.item>
    <x-ui.breadcrumbs.item>{{ $companyCreditAttribute->name ?: 'Company credit attribute' }}</x-ui.breadcrumbs.item>
@endsection
...
@section('content')
    <section class="space-y-6">
        ...
        <x-ui.card data-slot="company-credit-attribute-detail-hero" class="sb-page-hero !max-w-none p-6 sm:p-7">
            <div class="grid gap-6 xl:grid-cols-[minmax(0,1.04fr)_minmax(18rem,0.96fr)] xl:items-start">
                <div class="space-y-5">
                    <div class="flex flex-wrap items-center gap-2">
                        <div class="sb-page-kicker">Company metadata archive</div>
                    ...
                    <div class="space-y-3">
                        <x-ui.heading level="h1" size="xl" class="sb-page-title">
                            {{ $companyCreditAttribute->name ?: 'Company credit attribute' }}
                        </x-ui.heading>

                        <x-ui.text class="sb-page-copy max-w-4xl text-base">
                            Published company credit records using this attribute, including linked companies, categories, territories, and the titles they attach to.
                        </x-ui.text>
                    </div>
                </div>
        ...
        <x-ui.card data-slot="company-credit-attribute-detail-filters" class="sb-detail-section !max-w-none">
            <form method="GET" action="{{ route('public.company-credit-attributes.show', $companyCreditAttribute) }}" class="grid gap-4 xl:grid-cols-[minmax(0,1fr)_12rem_14rem_14rem_14rem_auto] xl:items-end">
                <x-ui.field>
                ...
                <x-ui.field>
                    ...
                    >
                        ...
                        @foreach ($companyOptions as $companyOption)
                            <option value="{{ $companyOption['value'] }}" @selected($filters['company'] === $companyOption['value'])>
                                {{ $companyOption['label'] }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="flex flex-wrap gap-2">
                    ...
                    @if ($hasActiveFilters)
                        <x-ui.button as="a" :href="route('public.company-credit-attributes.show', $companyCreditAttribute)" variant="ghost" icon="x-mark">
                            Clear
                        </x-ui.button>
                    @endif
                </div>
        ...
        <x-ui.card data-slot="company-credit-attribute-detail-records" class="sb-detail-section !max-w-none">
            <div class="space-y-4">
                <div>
                    <x-ui.heading level="h2" size="lg">Related company records</x-ui.heading>
                    <x-ui.text class="mt-1 text-sm text-neutral-600 dark:text-neutral-300">
                        Published titles that currently expose this company credit attribute in the imported catalog.
                    </x-ui.text>
                </div>
                ...
                <div class="space-y-3">
                                ...
                                <div class="min-w-0 space-y-3">
                                    <div class="space-y-2">
                                        ...
                                        <div class="flex flex-wrap gap-2 text-sm text-neutral-600 dark:text-neutral-300">
                                            ...
                                            @if ($archiveRecord['companyHref'] && $archiveRecord['companyLabel'])
                                                <a href="{{ $archiveRecord['companyHref'] }}">
                                                    <x-ui.badge variant="outline" color="neutral" icon="building-office-2">
                                                        {{ $archiveRecord['companyLabel'] }}
                                                    </x-ui.badge>
                                                </a>
                                            @elseif ($archiveRecord['companyLabel'])
                                                <x-ui.badge variant="outline" color="neutral" icon="building-office-2">
                                                    {{ $archiveRecord['companyLabel'] }}
                                                </x-ui.badge>
                                            @endif

                                            @if ($archiveRecord['categoryHref'] && $archiveRecord['categoryLabel'])
                                                <a href="{{ $archiveRecord['categoryHref'] }}">
                                                    <x-ui.badge variant="outline" color="slate" icon="tag">
                                                        {{ $archiveRecord['categoryLabel'] }}
                                                    </x-ui.badge>
                                                </a>
                                            @elseif ($archiveRecord['categoryLabel'])
                                                <x-ui.badge variant="outline" color="slate" icon="tag">
                                                    {{ $archiveRecord['categoryLabel'] }}
                                                </x-ui.badge>
                                            @endif
                                        ...
                                        <div class="flex flex-wrap gap-2">
                                            @forelse ($archiveRecord['attributeLinks'] as $attributeLink)
                                                <a
                                                    ...
                                                    class="inline-flex items-center rounded-full border px-2.5 py-1 text-xs font-medium transition {{ $attributeLink['id'] === $companyCreditAttribute->getKey() ? 'border-amber-300/70 bg-amber-50 text-amber-900 dark:border-amber-500/40 dark:bg-amber-500/10 dark:text-amber-100' : 'border-black/8 text-neutral-700 hover:bg-white dark:border-white/10 dark:text-neutral-200 dark:hover:bg-white/[0.05]' }}"
                                                >
                                                    {{ $attributeLink['label'] }}
                                                </a>
                                            @empty
                    ...
                    @empty
                        <div class="rounded-[1.2rem] border border-dashed border-black/10 bg-white/50 p-6 text-sm text-neutral-600 dark:border-white/10 dark:bg-white/[0.02] dark:text-neutral-300">
                            No published titles match the current company credit attribute filters.
                        </div>
                    @endforelse
                </div>
