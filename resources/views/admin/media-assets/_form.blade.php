...
<div class="grid gap-4 lg:grid-cols-2">
    ...
    <x-ui.field>
        ...
        <x-ui.input name="alt_text" :value="old('alt_text', $mediaAsset->alt_text)" />
    ...
    <x-ui.field>
        ...
        <x-ui.input name="provider_key" :value="old('provider_key', $mediaAsset->provider_key)" />
        <x-ui.error name="provider_key" />
    </x-ui.field>
    <x-ui.field>
        <x-ui.label>Language</x-ui.label>
        <x-ui.input name="language" :value="old('language', $mediaAsset->language)" />
        <x-ui.error name="language" />
    </x-ui.field>
    <x-ui.field>
        <x-ui.label>Width</x-ui.label>
        <x-ui.input name="width" type="number" min="1" :value="old('width', $mediaAsset->width)" />
        <x-ui.error name="width" />
    </x-ui.field>
    <x-ui.field>
        <x-ui.label>Height</x-ui.label>
        <x-ui.input name="height" type="number" min="1" :value="old('height', $mediaAsset->height)" />
    ...
    <x-ui.field>
        ...
        <x-ui.input name="position" type="number" min="0" :value="old('position', $mediaAsset->position)" />
        <x-ui.error name="position" />
    </x-ui.field>
    <x-ui.field>
        <x-ui.label>Published at</x-ui.label>
        <x-ui.input name="published_at" type="datetime-local" :value="old('published_at', $mediaAsset->published_at?->format('Y-m-d\\TH:i'))" />
        <x-ui.error name="published_at" />
    </x-ui.field>
    <label class="flex items-center gap-2 text-sm lg:self-end">
        <input type="hidden" name="is_primary" value="0">
        <input type="checkbox" name="is_primary" value="1" class="rounded border-black/20 dark:border-white/20 dark:bg-neutral-900" @checked(old('is_primary', $mediaAsset->is_primary))>
...
<x-ui.field>
    ...
    <x-ui.textarea name="caption" rows="3">{{ old('caption', $mediaAsset->caption) }}</x-ui.textarea>
    <x-ui.error name="caption" />
</x-ui.field>

<x-ui.field>
    <x-ui.label>Metadata JSON</x-ui.label>
    <x-ui.textarea name="metadata" rows="5">{{ old('metadata', $mediaAsset->metadata ? json_encode($mediaAsset->metadata, JSON_PRETTY_PRINT) : null) }}</x-ui.textarea>
    <x-ui.text class="mt-1 text-xs text-neutral-500 dark:text-neutral-400">
