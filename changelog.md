...
### What's New
The catalog now has a dedicated public AKA attribute archive, so people can open an AKA marker like "Alternative title" and immediately browse the titles, countries, languages, and alternate-title records tied to it. Title detail pages also grew into much more human-friendly editorial pages, with better genre cards, interest-category links, featured cast cards, cleaner certificate sections, and improved media archive navigation. On top of that, the account and admin areas now have proper sidebar-based portal shells instead of feeling like the public site with a few shortcuts bolted on.

### What Was Improved
Livewire page payloads were normalized so empty collections and optional values stop leaking awkward edge cases into views. Shared UI controls were tightened up too: buttons now expose a consistent loading state, flag icons are resolved through a dedicated action instead of ad hoc Blade logic, and native selects can be reused across moderation screens. There is also an important import-side fix that deduplicates company-credit bridge rows before insert, which protects company country and attribute relationships from duplicate writes.
...
- `app/Actions/Catalog/LoadTitleDetailsAction.php` — expanded title-page payloads with humanized AKA, certificate, genre, and interest-category entries.
...
- `resources/views/companies/show.blade.php` — aligned the company page with normalized collection data.
...
- `routes/web.php` — registered the public AKA attribute archive route.
