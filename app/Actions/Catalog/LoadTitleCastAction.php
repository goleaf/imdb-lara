...
class LoadTitleCastAction
{
    ...
    public function handle(Title $title): array
    {
        ...
        if (! Credit::catalogCreditsAvailable()) {
            $castCredits = $this->emptyCreditsPaginator('cast_page');
            $crewCredits = $this->emptyCreditsPaginator('crew_page');
    ...
    private function emptyCreditsPaginator(string $pageName): LengthAwarePaginator
    {
        return new PaginationLengthAwarePaginator(
            [],
            0,
            self::PAGE_SIZE,
            PaginationLengthAwarePaginator::resolveCurrentPage($pageName),
            [
                'pageName' => $pageName,
