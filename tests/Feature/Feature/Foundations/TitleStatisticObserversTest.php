...
class TitleStatisticObserversTest extends TestCase
{
    ...
    public function test_rating_events_keep_title_statistics_in_sync(): void
    {
        ...
        $this->assertTitleStatistics($title, 1, '8.00', 0, 0, ['8' => 1]);

        $secondRating = Rating::factory()->for($title)->create(['score' => 6]);

        $this->assertTitleStatistics($title, 2, '7.00', 0, 0, ['8' => 1, '6' => 1]);

        $firstRating->update(['score' => 10]);

        $this->assertTitleStatistics($title, 2, '8.00', 0, 0, ['10' => 1, '6' => 1]);

        $secondRating->delete();

        $this->assertTitleStatistics($title, 1, '10.00', 0, 0, ['10' => 1]);
    }
